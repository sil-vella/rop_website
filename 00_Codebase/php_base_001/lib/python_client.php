...
/**
 * POST JSON to Python API without auth (for public endpoints).
 *
 * @param string $baseUrl PYTHON_API_BASE_URL (no trailing slash)
 * @param string $path Path to append (e.g. /dutch/register-for-tournament)
 * @param array $body JSON body as array
 * @return array{status: int, body: string} HTTP status and response body
 */
function python_post_public(string $baseUrl, string $path, array $body): array
{
    $url = $baseUrl . $path;
    $json = json_encode($body);
    if ($json === false) {
